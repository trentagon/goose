...
def modern_ss(inp,ode_solver):
    """
    This function finds the steady state solution for the modern Earth
    """
    ...
    pCO2_track = inp.modern_pCO2 # speeds up first guess

    # Create steady state checker
    event = lambda t, x: steadyStateCheck(t, x)
    event.terminal = True

    max_duration = 1e7 # maximum time before termination by default
...
def masterODE(t,x,inp,full_output):
    ...
    while 1:
        ...
        Ts = cm.climate_model_JKT(pCO2_o, 0.9906)  # calculate surface temperature - luminosity = 1
    ...
    W_carb = inp.modern_W_carb * ((pCO2_o / inp.modern_pCO2) ** inp.carb_xi) * np.exp(
        (Ts - inp.modern_Ts) / inp.cont_Te) # continental carbonate weathering flux

    W_sil = inp.modern_W_sil * ((pCO2_o / inp.modern_pCO2) ** inp.sil_alpha) * np.exp(
    ...
    if omega_o > 1: # ocean carbonates only precipitate if ocean omega is larger than 1
        P_shelf = inp.k_shelf * (omega_o-1)**inp.carb_n # carbonate precipitation flux on continental shelf
        P_pel = inp.k_pel * omega_o ** 2.84 # carbonate precipitation flux in open ocean
    else:
