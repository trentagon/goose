...
def neoproterozoic_ss(inp,ode_solver):
    """
    This function finds the steady state solution for the background Neoproterozoic parameters before the Snowball starts
    """

    # Need an initial guess to start the model - use the modern values from calibration
    Co = inp.modern_Co 
    Cp = inp.modern_Cp 
    Ao = inp.modern_Ao 
    Ap = inp.modern_Ap 

    initial_conditions = np.array([Co, Ao, Cp, Ap]) # put initial conditions into array for the ODE solver
    ...
    pCO2_track = 2000e-6 # speeds up first guess

    # Create steady state checker
    event = lambda t, x: steadyStateCheck(t, x)
    event.terminal = True

    max_duration = 1e8 # maximum time before termination by default
    ...
    # all_results = masterODE(0,res.y[:,-1],inp,1)
    # all_results.input_file = inp
    # all_results.res = res

    #Define data frame
    r = masterODE(0,res.y[:,-1],inp,1)
    all_results = ODE_full_output()
    column_labels = ["t","Co","Ao","Cp","Ap","Ts","Td","Tp","pCO2_o","pCO2_p","Omega_o","Omega_p","pH_o","pH_p","V","P_ocean","P_shelf","P_pore","W_carb","W_sil","W_sea"]
    row_results = [-1,r.Co,r.Ao,r.Cp,r.Ap,r.Ts,r.Td,r.Tp,r.pCO2_o,r.pCO2_p,r.omega_o,r.omega_p,r.pH_o,r.pH_p,r.V,r.P_ocean,r.P_shelf,r.P_pore,r.W_carb,r.W_sil,r.W_sea]
    all_results.data = pd.DataFrame(columns=column_labels)
    all_results.data.loc[0] = row_results
...
def masterODE(t,x,inp,full_output):
    ...
    dCo_dt = (1 / inp.modern_ocean_mass) * ((-J*(DIC_o - Cp)) + V + W_carb - P_ocean) # ocean carbon ODE
    dAo_dt = (1 / inp.modern_ocean_mass) * ((-J * (Ao - Ap)) + (2*W_carb) + (2*W_sil) - (2*P_ocean)) # ocean alkalinity ODE
