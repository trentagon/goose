...
def poststrat_evol(inp,ode_solver,initial_conditions, max_duration):
    """
    ...
    md = max_duration # this is how managed to get it in there

    res = solve_ivp(lambda t, x: masterODE(t, x, inp, 0), (0, max_duration), initial_conditions,  method=ode_solver, max_step = 100) # solve the ODE and check for steady state
...
def masterODE(t,x,inp,full_output):
    ...
    while 1:
        ...
        Ts = cm.climate_model_JKT(pCO2_s, inp.neoproterozoic_luminosity)  # calculate surface temperature
        omega_s, pCO2_s, pH_s = cm.equilibriumChemistryOcean(Ts, As, Cs, inp.meltwater_s, inp.meltwater_alk, inp.meltwater_Ca)  # calculate surface ocean chemistry
    ...
    W_carb = inp.f_w * inp.modern_W_carb * ((pCO2_s / inp.modern_pCO2) ** inp.carb_xi) * np.exp(
        (Ts - inp.modern_Ts) / inp.cont_Te) # continental carbonate weathering flux

    W_sil = inp.f_w * inp.modern_W_sil * ((pCO2_s / inp.modern_pCO2) ** inp.sil_alpha) * np.exp(
    ...
    DIC_s = Cs - (inp.meltwater_s*pCO2_s) # subtract off the atmosphere because that carbon is not mixing with the pore space

    dCs_dt = (1 / inp.meltwater_mass) * ((-K*(DIC_s - Cd)) + V_air + W_carb - P_ocean - O_sink) # surface ocean carbon ODE
    dAs_dt = (1 / inp.meltwater_mass) * ((-K * (As - Ad)) + (2*W_carb) + (2*W_sil) - (2*P_ocean)) # surface ocean alkalinity ODE
